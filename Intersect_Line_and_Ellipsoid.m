...
% Now, using the matlab root function we can compute the number of roots:
rts = roots([A B C]);

% if roots are unequal and real, there are two solutions
if (rts(1) == real(rts(1)) && rts(1) ~= rts(2))
    int1 = P + rts(1)*v; % find intersection point using first root t
    int2 = P + rts(2)*v; % find intersection point using second root t
    num_ints = 2; % having passed condition D > 0, we know there are 2 roots
    
    % order points of intersection by lowest 't' then highest 't'
    if rts(1) < rts(2)
...
% if roots are real but equal, one solution
elseif (rts(1) == real(rts(1)))
    num_ints = 1; % Here, we know there is only 1 unique intersection 
    intersecs = P + rts(1)*v; % find single intersection point
